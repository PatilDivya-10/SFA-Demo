...
app.secret_key = (
    ...
    or 'smart-fashion-advisor-dev-only'
)
...
def add_no_cache_headers(response):
    """Prevent browser from serving stale JS/HTML during rapid iterations."""
...
FUNCTION_INTENT = {
    ...
    'process_image': 'Detects face regions and classifies skin tone from balanced image values.',
    'classify_skin_tone': 'Maps RGB values to skin-tone categories using the ITA metric.',
...
def normalize_gender(value):
    """Normalize incoming gender values to supported keys."""
    if not value:
        return 'male'
...
def normalize_skin_tone(value):
    """Normalize skin tone input to a known palette key."""
    if not value:
        return 'Medium'
    tone = str(value).strip()
    return tone if tone in SKIN_TONE_COLORS else 'Medium'


def json_safe(value):
    """Convert pandas/numpy scalars to native Python types for JSON responses."""
    if value is None:
        return None
...
def sanitize_outfit_item(item):
    """Ensure outfit dict values are JSON-serializable strings/numbers."""
    clean = {}
    for key, val in item.items():
        if key in ('id', 'source_index', 'score'):
            try:
                clean[key] = int(json_safe(val) or 0)
            except (TypeError, ValueError):
                clean[key] = 0
        else:
            text = json_safe(val)
            clean[key] = '' if text is None else str(text)
...
if not os.path.isfile(DATASET_PATH):
    LOGGER.error(
        "Missing %s — outfit recommendations will be empty. "
        "Ensure data/styles.csv is in the repo for Railway/GitHub deploy.",
        DATASET_PATH,
    )
...
class HeadlessSkinToneDetector:
    ...
    def prepare_image(self, bgr_image):
        """Normalize image size for reliable face detection."""
    ...
    def check_lighting_quality(self, frame):
        ...
        if mean_brightness < 12:
            return False, 'Image is too dark. Please move to a brighter area or turn on a light.'
        if mean_brightness > 252 and std_brightness < 8:
            return False, 'Image is overexposed. Please reduce direct light on your face.'
        return True, None

    def detect_face_landmarks(self, bgr_image):
        """Try multiple preprocessing passes to improve face detection."""
        attempts = [
            bgr_image,
            self.gray_world_white_balance(bgr_image),
            self.normalize_lighting(bgr_image),
            cv2.convertScaleAbs(bgr_image, alpha=1.12, beta=8),
        ]
        seen = set()
        for candidate in attempts:
            ...
            if results.multi_face_landmarks:
                return results.multi_face_landmarks[0], candidate
        return None, bgr_image
    ...
    def extract_skin_color(self, frame, skin_regions):
        ...
        ycrcb_frame = cv2.cvtColor(normalized_frame, cv2.COLOR_BGR2YCrCb)

        all_skin_pixels = []

        for points in skin_regions.values():
            ...
            region_ycrcb = ycrcb_frame[y:y + h, x:x + w]

            hsv_mask = cv2.inRange(region_hsv, np.array([0, 20, 35], dtype=np.uint8), np.array([35, 180, 255], dtype=np.uint8))
            ycrcb_mask = cv2.inRange(region_ycrcb, np.array([0, 133, 77], dtype=np.uint8), np.array([255, 173, 127], dtype=np.uint8))
        ...
        rgb_vals = self._trim_skin_pixels(all_skin_pixels)
        if rgb_vals is not None:
            return rgb_vals, None
        return None, None

    def extract_skin_from_landmarks(self, frame, skin_regions):
        """Fallback: sample pixels inside landmark convex hulls when masks fail."""
        normalized_frame = self.normalize_lighting(frame)
        collected = []
        for points in skin_regions.values():
            hull = cv2.convexHull(points.astype(np.int32))
            mask = np.zeros(normalized_frame.shape[:2], dtype=np.uint8)
            cv2.fillConvexPoly(mask, hull, 255)
            pixels = normalized_frame[mask > 0]
            if len(pixels) > 0:
                collected.extend(pixels)
        return self._trim_skin_pixels(collected)
    ...
    def process_image(self, bgr_image):
        prepared = self.prepare_image(bgr_image)
        if prepared is None:
            return {"success": False, "message": "Invalid image. Please upload a JPG or PNG photo."}

        lighting_ok, lighting_message = self.check_lighting_quality(prepared)
        balanced = self.gray_world_white_balance(prepared)
        if not lighting_ok:
            balanced = self.normalize_lighting(balanced)
            LOGGER.debug("Continuing analysis after lighting warning: %s", lighting_message)

        landmarks, balanced = self.detect_face_landmarks(balanced)
        if landmarks is None:
            return {
                "success": False,
                "message": "No face detected. Center your face in the frame with good lighting, then try again.",
            }

        skin_regions = self.get_skin_regions(landmarks, balanced.shape)
        rgb_vals, _ = self.extract_skin_color(balanced, skin_regions)
        if rgb_vals is None:
            rgb_vals = self.extract_skin_from_landmarks(balanced, skin_regions)

        if rgb_vals is None:
            return {
                "success": False,
                "message": "Could not extract skin tone. Please retake the photo with your face clearly visible.",
            }
        ...
        return {
            ...
            "lighting_warning": None if lighting_ok else lighting_message,
        }
...
_OUTFIT_RESULTS_CACHE: dict = {}
_EMPTY_OUTFIT_RESULTS = {
    'topwear': [],
    'bottomwear': [],
    'footwear': [],
    'dress': [],
...
def get_scored_fashion_candidates(skin_tone, gender, occasion=None):
    """Return scored candidate items for recommendation and evaluation."""
    if fashion_data.empty:
        return pd.DataFrame()

    gender = normalize_gender(gender)
    skin_tone = normalize_skin_tone(skin_tone)
    cache_key = (skin_tone, gender)
    if cache_key in _SCORED_CATALOG_CACHE:
        ...
        gender_map = {'male': 'Men', 'female': 'Women'}
        target_gender = gender_map.get(gender, 'Men')
...
def get_recommended_outfits(skin_tone, gender, occasion=None, limit=12):
    """Get outfit recommendations based on skin tone and preferences"""
    skin_tone = normalize_skin_tone(skin_tone)
    gender = normalize_gender(gender)
    occasion_key = (occasion or '').strip().lower()
    cache_key = (skin_tone, gender, occasion_key, limit)
    if cache_key in _OUTFIT_RESULTS_CACHE:
        return _OUTFIT_RESULTS_CACHE[cache_key]

    scored_data = get_scored_fashion_candidates(skin_tone, gender, occasion)
    if scored_data.empty:
        return dict(_EMPTY_OUTFIT_RESULTS)
    filtered_data = scored_data[scored_data['color_score'] >= 1]
    if filtered_data.empty:
        filtered_data = scored_data.head(500)
    if filtered_data.empty:
        return dict(_EMPTY_OUTFIT_RESULTS)

    results = {key: [] for key in _EMPTY_OUTFIT_RESULTS}
    per_category_target = min(limit, 8)
    max_rows_to_scan = min(len(filtered_data), max(per_category_target * 400, 1800))
    ...
    def _enough_results():
        core_filled = sum(1 for cat in ('topwear', 'bottomwear', 'footwear', 'dress') if len(results[cat]) >= per_category_target)
        if core_filled >= 3:
            return True
        return sum(len(items) for items in results.values()) >= per_category_target * 5

    for scan_idx, (idx, row) in enumerate(filtered_data.iloc[:max_rows_to_scan].iterrows()):
        if _enough_results():
            break
        if scan_idx > 0 and scan_idx % 250 == 0 and _enough_results():
            break
        ...
        for keyword in category_keywords['traditional']:
            if keyword in combined_text and len(results['traditional']) < per_category_target:
        ...
        if not categorized:
            for keyword in category_keywords['outerwear']:
                if keyword in combined_text and len(results['outerwear']) < per_category_target:
        ...
        if not categorized:
            for keyword in category_keywords['topwear']:
                if keyword in combined_text and len(results['topwear']) < per_category_target:
        ...
        if not categorized:
            for keyword in category_keywords['bottomwear']:
                if keyword in combined_text and len(results['bottomwear']) < per_category_target:
        ...
        if not categorized:
            for keyword in category_keywords['footwear']:
                if keyword in combined_text and len(results['footwear']) < per_category_target:
        ...
        if not categorized:
            for keyword in category_keywords['dress']:
                if keyword in combined_text and len(results['dress']) < per_category_target:
        ...
        if not categorized:
            for keyword in category_keywords['accessories']:
                if keyword in combined_text and len(results['accessories']) < per_category_target:
                    results['accessories'].append(item)
                    categorized = True
                    break

    _OUTFIT_RESULTS_CACHE[cache_key] = results
    return results
...
def analyze_image():
    ...
    try:
        ...
        if img is None:
            return jsonify({'success': False, 'message': 'Could not decode image. Please use JPG or PNG format.'}), 400

        LOGGER.debug("Processing uploaded image for skin-tone analysis (%s)", img.shape)
        result = detector.process_image(img)
        LOGGER.debug("Image analysis result: %s", {k: v for k, v in result.items() if k != 'rgb'})
        ...
        LOGGER.exception("Analyze image failed: %s", e)
        return jsonify({
            'success': False,
            'message': 'Server error while analyzing the photo. Please try again in a moment.',
        }), 500
...
def build_metrics_plot_base64(metrics):
    ...
    colors = ['#4e79a7', '#59a14f', '#f28e2b', '#e15759', '#9c755f', '#bab0ac']
    fig, ax = plt.subplots(figsize=(9.5, 4.8))
    bars = ax.bar(labels, values, color=colors)
    ax.set_ylim(0, 1)
    ax.set_ylabel('Value (0-1)')
    ax.set_title('Recommendation Metrics (computed for this run)')
...
def evaluate_recommendation_quality(
    ...
    mode:
      - recommender: predicted positives are items returned by recommender (UI-like).
      - classifier: predicted positives are derived from internal score threshold (color_score>=2).
    """
    scored_data = get_scored_fashion_candidates(skin_tone, gender, occasion)
    if scored_data.empty:
        return {
            'metrics': {'accuracy': 0.0, 'precision': 0.0, 'recall': 0.0, 'f1_score': 0.0, 'mae': 0.0, 'rmse': 0.0},
    ...
    if mode == "classifier":
        # Evaluate the scoring rule itself (not limited by how many items we display).
        # Use a stricter threshold than the label-definition threshold to avoid trivial 1.0 metrics.
        thresh = int(score_threshold) if score_threshold is not None else 3
        scored_eval['predicted_positive'] = (scored_eval.get('color_score', 0).fillna(0) >= thresh).astype(int)
        recommended_source_indices = set(scored_eval.index[scored_eval['predicted_positive'] == 1].tolist())
        predicted_what = f"score_threshold(color_score>={thresh})"
    else:
        recommendations = get_recommended_outfits(skin_tone, gender, occasion, limit)
        ...
        predicted_what = f"recommender_top_items(limit={limit})"
    # Regression-style error metrics from internal scoring:
    # predicted_score is a normalized proxy signal (0..1) derived from color_score (0..3).
    if 'color_score' in scored_eval.columns:
        predicted_score = (scored_eval['color_score'].fillna(0).clip(0, 3).astype(float) / 3.0)
    else:
        predicted_score = scored_eval['predicted_positive'].astype(float)
    actual_score = scored_eval['actual_positive'].astype(float).clip(0, 1)
    abs_err = (predicted_score - actual_score).abs()
    sq_err = (predicted_score - actual_score) ** 2
    mae = float(abs_err.mean()) if len(abs_err) else 0.0
    rmse = float(np.sqrt(sq_err.mean())) if len(sq_err) else 0.0
    ...
    details = (
        ...
        f"Mode={mode} predicted_by={predicted_what}. "
        f"From {len(scored_eval)} candidate products, {len(recommended_source_indices)} were predicted positive. "
        ...
        f"MAE ({metrics['mae']}) and RMSE ({metrics['rmse']}) summarize average error magnitude using normalized internal scores."
    )
...
def get_outfits():
    """Lightweight outfit list for the recommendations page."""
    data = request.get_json(silent=True) or {}
    skin_tone = normalize_skin_tone(data.get('skin_tone', session.get('skin_tone', 'Medium')))
    gender = normalize_gender(data.get('gender', session.get('gender', 'male')))
    ...
    try:
        outfits = get_recommended_outfits(skin_tone, gender, occasion)
        total = sum(len(items) for items in outfits.values())
        return jsonify({
            'success': True,
            'outfits': outfits,
            'count': total,
            'message': None if total else 'No outfits found for this occasion. Try All Occasions.',
        })
    except Exception as exc:
        LOGGER.exception("Outfit loading failed: %s", exc)
        return jsonify({
            'success': False,
            'outfits': dict(_EMPTY_OUTFIT_RESULTS),
            'count': 0,
            'message': 'Could not load outfits. Please try again.',
        })


@app.route('/api/recommendations', methods=['POST'])
def get_recommendations():
    """Get outfit recommendations (palette + combinations + optional evaluation)."""
    reset_execution_trace()
    data = request.get_json(silent=True) or {}
    skin_tone = normalize_skin_tone(data.get('skin_tone', session.get('skin_tone', 'Medium')))
    gender = normalize_gender(data.get('gender', session.get('gender', 'male')))
    occasion = data.get('occasion')
    include_evaluation = bool(data.get('include_evaluation', False))
    include_trace = bool(data.get('include_trace', False))
    evaluation_mode = str(data.get('evaluation_mode', 'recommender')).strip().lower()
    score_threshold = data.get('score_threshold', 3)

    session['gender'] = gender
    session['skin_tone'] = skin_tone

    recommendations = dict(_EMPTY_OUTFIT_RESULTS)
    outfit_combinations = []
    evaluation = {
        'metrics': {'accuracy': 0.0, 'precision': 0.0, 'recall': 0.0, 'f1_score': 0.0},
        'confusion_matrix': {'tp': 0, 'tn': 0, 'fp': 0, 'fn': 0},
        'detailed_text': 'Fast mode enabled. Detailed recommendation metrics are skipped for quicker loading.',
        'graph_base64': None,
    }
    error_message = None

    try:
        recommendations = get_recommended_outfits(skin_tone, gender, occasion)
        outfit_combinations = get_outfit_combinations(skin_tone)
        if include_evaluation:
            evaluation = evaluate_recommendation_quality(
                skin_tone,
                gender,
                occasion,
        ...
        error_message = 'Some recommendation data could not be loaded. Showing available results.'
...
def health():
    ...
    return jsonify({
        'status': 'ok',
        'version': APP_VERSION,
        'catalog_loaded': not fashion_data.empty,
        'catalog_items': int(len(fashion_data)),
    }), 200
...
if __name__ == '__main__':
    ...
    LOGGER.info('Smart Fashion Advisor v%s', APP_VERSION)
    LOGGER.info('Catalog: %d items | Images: %d URLs', len(fashion_data), len(image_urls))
    LOGGER.info('Open in browser: http://127.0.0.1:%s', port)
